...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, name, price, SUM(shares) as allShares FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
    cash = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]

    total = cash

    for stock in stocks:
        total += stock["price"] * stock["allShares"]
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("Please enter a symbol ex) MSFT")
        elif not stock:
            return apology("Please enter a valid symbol ex) MSFT")
...
def quote():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("Please enter symbol ex) MSFT")
